...
export function QRTicket({
  ...
  const downloadTicket = () => {
    ...
    const file = new Blob([JSON.stringify(ticketData, null, 2)], {
      type: "text/plain",
    });
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm">
                  {getPurposeDisplay(ticketData.purpose)}
                </span>
              </div>
              {ticketData.books && ticketData.books.length > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="text-sm">
                    {ticketData.books.length} book(s)
                  </span>
                </div>
              )}
              {ticketData.equipment && ticketData.equipment.length > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="text-sm">
                    {ticketData.equipment.length} item(s)
                  </span>
                </div>
              )}
              {ticketData.expiresAt && (
                <div className="flex justify-between">
                  ...
                  <span
                    className={`text-sm ${
                      ticketData.status === "expired" ? "text-red-600" : ""
                    }`}
                  >
      ...
      {((ticketData.books && ticketData.books.length > 0) ||
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Books and equipment included in this QR ticket
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {/* Books */}
              {ticketData.books &&
                ticketData.books.map((book: any, index: number) => (
                  <div
                    key={`book-${book.id}`}
              ...
              {ticketData.equipment &&
                ticketData.equipment.map((item: any, index: number) => (
                  <div
