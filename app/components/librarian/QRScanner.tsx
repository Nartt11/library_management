...
// import { QrCode, Camera, CheckCircle, XCircle, User, BookOpen, Calendar, Clock, AlertTriangle } from 'lucide-react';
...
// export function QRScanner({ user }: QRScannerProps) {
...
//   const [scanAlertType, setScanAlertType] = useState<'scan' | 'attendance'>('scan');
...
//         ticketId: `CHK-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
//         studentId: 'ST-001',
//         studentName: 'Alex Johnson',
//         books: [
//           {
...
//         toast.error('This checkout ticket has expired. Student needs to generate a new one.');
//         return;
//       }
//     }
...
//     toast.success(`Checkout completed! ${lastScanResult.books.length} book(s) borrowed by ${lastScanResult.studentName}`);
//   };
...
//       studentName: lastScanResult.studentName,
//       studentId: lastScanResult.studentId,
//       books: lastScanResult.books?.map((book: any) => book.title) || ['Sample Book Title'],
//       timestamp: new Date().toLocaleString(),
...
//           QR Code Scanner
//         </h1>
//         <p className="text-muted-foreground">Scan student QR tickets for borrowing, returns, and attendance.</p>
//       </div>
...
//             <CardDescription>Scan QR codes or enter ticket IDs manually</CardDescription>
//           </CardHeader>
...
//             <CardDescription>Student information and available actions</CardDescription>
//           </CardHeader>
//           <CardContent>
//             {lastScanResult ? (
...
//                     <span className="text-red-800">Expired checkout ticket</span>
//                   </div>
//                 ) : (
...
//                       <p className="text-xs text-muted-foreground">ID: {lastScanResult.studentId}</p>
//                     </div>
//                   </div>

//                   <div className="text-xs text-muted-foreground">
//                     <p>Ticket ID: {lastScanResult.ticketId || lastScanResult.id}</p>
//                     <p>Generated: {new Date(lastScanResult.checkoutTime || lastScanResult.timestamp).toLocaleString()}</p>
//                     {lastScanResult.expiresAt && (
//                       <div className="flex items-center gap-1">
//                         <Clock className="h-3 w-3" />
//                         <span>{getTimeRemaining(lastScanResult.expiresAt)}</span>
//                       </div>
//                     )}
//                   </div>
//                 </div>
...
//                   <div className="space-y-2">
//                     <h4 className="text-sm">Books ({lastScanResult.books.length}):</h4>
//                     <div className="max-h-24 overflow-y-auto space-y-1">
//                       {lastScanResult.books.map((book: any, index: number) => (
//                         <div key={index} className="text-xs text-muted-foreground">
//                           {index + 1}. {book.title || book} {book.author ? `by ${book.author}` : ''}
//                         </div>
//                       ))}
//                     </div>
//                   </div>
...
//                       </Button>
//                     ) : (
//                       <Button onClick={processBorrow} size="sm" className="justify-start gap-2">
//                         <BookOpen className="h-4 w-4" />
//                         Process Book Borrowing
//                       </Button>
//                     )}
//                     <Button onClick={processReturn} size="sm" variant="outline" className="justify-start gap-2">
//                       <BookOpen className="h-4 w-4" />
//                       Process Book Return
//                     </Button>
//                     <Button onClick={processAttendance} size="sm" variant="outline" className="justify-start gap-2">
...
//                 <p className="text-sm text-muted-foreground">Scan a QR code or enter ticket ID manually</p>
//               </div>
//             )}
//           </CardContent>
//         </Card>
...
//           <CardDescription>Recent QR code scans and transactions</CardDescription>
//         </CardHeader>
//         <CardContent>
//           <div className="space-y-3">
...
//                     <p className="text-sm truncate">{scan.studentName} ({scan.studentId})</p>
//                     {scan.books.length > 0 && (
//                       <p className="text-xs text-muted-foreground truncate">
//                         {scan.books.length} book(s): {scan.books.join(', ')}
//                       </p>
//                     )}
//                     <p className="text-xs text-muted-foreground">Processed by: {scan.processedBy}</p>
//                   </div>
//                 </div>
